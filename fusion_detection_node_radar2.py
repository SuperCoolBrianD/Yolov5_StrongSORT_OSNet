...
class Camera_Detection(Node):
    def __init__(self):
        ...
        self.t = 0
        self.radar_half = False
        self.camera_half = False
        self.camera_time = 0
        self.radar_time = 0
    ...
    def radar_callback(self, msg):
        ...
        if self.radar_half:
            self.radar_queue.put(msg)
            self.radar_half = True
        else:
            self.radar_half = True
    ...
    def camera_callback(self, msg):
        if self.camera_half:
            self.camera_queue.put(msg)
            self.camera_half = True
        else:
            self.camera_half = True
    ...
    def fusion(self):
        while True:
            ...
            if self.camera_queue.qsize() > 0 and self.radar_queue.qsize()> 0:
                print(f"camera is {self.camera_queue.qsize()}")
                print(f"radar is {self.radar_queue.qsize()}")
                if self.camera_queue.qsize()>0:
                    c = self.camera_queue.get(block=False)
                    self.frame.load_data_camera(c)
                    #print("loaded camera")
                else:
                    print("empty camera")
                if self.radar_queue.qsize()>1:
                    r1 = self.radar_queue.get(block=False)
                    r2 = self.radar_queue.get(block=False)
            ...
            if self.frame.full_data:
                    ...
                    # print(self.t - radar_utils.msg_sortv2(camera_msg))
                    # self.t = radar_utils.msg_sortv2(camera_msg)
                ...
                # print('camera')
                # print(len(self.frame.camera))
                # print('radar')
                # print(len(self.frame.radar))
                self.frame.clear()
