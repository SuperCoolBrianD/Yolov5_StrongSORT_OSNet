    ...
    if i.topic == '/usb_cam/image_raw/compressed':
        ...
        if pc.any():
            ...
            for t in range(track_bbs_ids.shape[0]):
                if track_bbs_ids[t, -1] not in trk:
                    trk[track_bbs_ids[t, -1]] = [(track_bbs_ids[0], ct)]
                else:
                    trk[track_bbs_ids[t, -1]].append((track_bbs_ids[0], ct))
            for t in range(track_bbs_ids.shape[0]):
                if track_bbs_ids[t, -1] in trk:
                    if len(trk[track_bbs_ids[t, -1]]) > 1:
                        if trk[track_bbs_ids[t, -1]][-2][1]-ct < 2000:
                            print(track_bbs_ids[t, -1])
                            vx = (trk[track_bbs_ids[t, -1]][-1][0][0]-trk[track_bbs_ids[t, -1]][-2][0][0])\
                                 /(trk[track_bbs_ids[t, -1]][-1][1]-trk[track_bbs_ids[t, -1]][-2][1])*1000000
                            vy = (trk[track_bbs_ids[t, -1]][-1][0][1]-trk[track_bbs_ids[t, -1]][-2][0][1])\
                                 /(trk[track_bbs_ids[t, -1]][-1][1]-trk[track_bbs_ids[t, -1]][-2][1])*1000000
                            # for key in trk.keys():
                            #     print(key)
                            #     print(trk[key])
                            axs.text(track_bbs_ids[t, 0], track_bbs_ids[t, 1] + 4, f"vx: {vx :0.2f}, vy: {vy:0.2f}", fontsize=22, color='r')
